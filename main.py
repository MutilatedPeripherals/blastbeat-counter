...
def get_sections_labeled_by_percussion_content_from_audio(time, data, sample_rate, bass_drum_freq, snare_drum_freq)-> list[tuple[tuple[int, int], bool, bool]]:
    # TODO: refactor the return type, it's a hideous monstrosity
    results = []
...
def idenfity_blastbeat_intervals(sections: list[tuple[tuple[int, int], bool, bool]]) -> list[tuple[int,int]]:
    ...
    for i in range(0, len(sections)):
        # current_section =
        current_section_start = sections[i][0][0]
        contains_snare = sections[i][1]
        contains_bass_drum = sections[i][2]

        if contains_snare and contains_bass_drum:
            if hits_count == 0:
                blastbeat_start_idx = i
            hits_count = hits_count + 1
        else:
            if hits_count >= min_hits_threshold:
                start = sections[blastbeat_start_idx][0][0]
                results.append((start, current_section_start))
            hits_count = 0
...
if __name__ == "__main__":
    ...
    # TODO: make this function return snare_freq, bass_drum_freq
    ...
    blast_intervals = idenfity_blastbeat_intervals(labeled_sections)
    plot_waveform(time, audio_data, blast_intervals, title=file_path.stem)